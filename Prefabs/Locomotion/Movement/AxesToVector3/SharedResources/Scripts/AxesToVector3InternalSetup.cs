namespace VRTK.Core.Prefabs.Locomotion.Movement.AxesToVector3
{
    ...
    public class AxesToVector3InternalSetup : MonoBehaviour, IProcessable
    {
        ...
        /// <summary>
        /// The container that holsd the momemt processing.
        /// </summary>
        [Tooltip("The container that holsd the momemt processing.")]
        public GameObject momentProcessorContainer;
        #endregion
        ...
        public virtual void Process()
        {
            ...
            CurrentMovement = inputMask.Transform(axisDirection) * Time.deltaTime;
            facade.Processed?.Invoke(CurrentMovement);
        }

        /// <summary>
        /// Toggles the state of the <see cref="momentProcessorContainer"/>.
        /// </summary>
        /// <param name="state"></param>
        public virtual void ToggleProcessOnMoment(bool state)
        {
            momentProcessorContainer.SetActive(state);
        }
        ...
        {
            SetAxisSources();
            speedMultiplier.Transformed.AddListener(ProcessMovement);
            SetMultipliers();
            ToggleProcessOnMoment(facade.ProcessOnMoment);
        }

        protected virtual void OnDisable()
        {
            SetAxisSources(true);
            speedMultiplier.Transformed.RemoveListener(ProcessMovement);
        }
        ...
        protected virtual void ProcessMovement(Vector3 axisData)
        {
            if (facade == null)
            {
                return;
